...
// Subjects for the "Add Class" dropdown. Subjects are shared, school-wide
// data (see subjects.php) — any admin or faculty account can add one — so
// this must list ALL of them, not just ones the currently logged-in
// admin/faculty account happens to have created themselves. Filtering by
// user_id here was the bug that made a subject added by one admin/faculty
// account seem to "disappear" when a different admin/faculty went to add a
// class: it was never missing from the database, it just wasn't being
// queried. Students never see this dropdown at all.
...
<?php if ($canManageSchedule && !$hasSubjects): ?>
  <div class="card"><div class="empty-state">
    ...
    <div class="empty-sub">At least one subject needs to exist before you can add classes to the schedule. <a href="subjects.php" style="color:var(--glow2)">Go to Subjects →</a></div>
  </div></div>
<?php endif; ?>
