        ...
        if (!isset($_FILES['avatar']) || $_FILES['avatar']['error'] === UPLOAD_ERR_NO_FILE) {
            ...
            if (!$imgInfo || !in_array($ext, $allowedExt, true)) {
                ...
                if (move_uploaded_file($tmp, $destFs)) {
                    ...
                    // Post/Redirect/Get: send the browser to a fresh GET request
                    // after a successful upload. This keeps the "upload" POST out
                    // of browser history entirely, so there's nothing stale for
                    // the Back button to show and no "resubmit form?" prompt on
                    // refresh — every visit to profile.php after this always
                    // re-reads the picture straight from the database.
...
if (isset($_GET['saved']))   { $message = "Profile picture updated."; $msgType = "ok"; }
if (isset($_GET['removed'])) { $message = "Profile picture removed."; $msgType = "ok"; }
