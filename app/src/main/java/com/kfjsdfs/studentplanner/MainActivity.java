 ...
 * isn't cleared. No server and no PHP/MySQL are required. The app does
 * request internet access (see AndroidManifest.xml) so it can send the
 * optional Gmail due-date reminder emails via EmailJS — everything
 * else still works fully offline.
 *
 ...
 *
 * SURVIVING UNINSTALL: accounts and their data used to disappear if the
 * app was uninstalled, since that wipes the WebView's local storage
 * along with everything else. AndroidManifest.xml now points
 * android:fullBackupContent / android:dataExtractionRules at
 * res/xml/backup_rules.xml and res/xml/data_extraction_rules.xml,
 * which explicitly keep that storage in Android's own Auto Backup for
 * Apps. As long as phone backup is turned on (Settings > Google >
 * Backup) and the person logs in again with the same Gmail, their
 * account and its data come back automatically after a reinstall — on
 * the same phone or a new one signed into the same Google account.
 * Deleting an account from inside the app is still the only way to
